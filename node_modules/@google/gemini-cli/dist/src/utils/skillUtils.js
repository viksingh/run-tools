 ...
 */
export async function installSkill(source, scope, subpath, onLog, requestConsent = () => Promise.resolve(true)) {
    ...
    try {
        ...
        if (skills.length === 0) {
            throw new Error(`No valid skills found in ${source}${subpath ? ` at path "${subpath}"` : ''}. Ensure a SKILL.md file exists with valid frontmatter.`);
        }
 ...
 */
export async function linkSkill(source, scope, onLog, requestConsent = () => Promise.resolve(true)) {
    ...
    for (const skill of skills) {
        if (seenNames.has(skill.name)) {
            throw new Error(`Duplicate skill name "${skill.name}" found at multiple locations:\n  - ${seenNames.get(skill.name)}\n  - ${skill.location}`);
        }
