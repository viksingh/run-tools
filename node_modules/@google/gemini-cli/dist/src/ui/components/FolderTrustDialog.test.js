...
describe('FolderTrustDialog', () => {
    ...
    it('should render the dialog with title and description', () => {
        const { lastFrame, unmount } = renderWithProviders(_jsx(FolderTrustDialog, { onSelect: vi.fn() }));
    ...
    it('should display exit message and call process.exit and not call onSelect when escape is pressed', async () => {
        ...
        const { lastFrame, stdin, unmount } = renderWithProviders(_jsx(FolderTrustDialog, { onSelect: onSelect, isRestarting: false }));
        act(() => {
    ...
    it('should display restart message when isRestarting is true', () => {
        const { lastFrame, unmount } = renderWithProviders(_jsx(FolderTrustDialog, { onSelect: vi.fn(), isRestarting: true }));
    ...
    it('should not call process.exit when "r" is pressed and isRestarting is false', async () => {
        const { stdin, unmount } = renderWithProviders(_jsx(FolderTrustDialog, { onSelect: vi.fn(), isRestarting: false }));
        act(() => {
            stdin.write('r');
        });
        await waitFor(() => {
