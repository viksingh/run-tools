...
const renderWithContext = (ui, streamingStateValue, width = 120) => {
    ...
    return render(_jsx(StreamingContext.Provider, { value: contextValue, children: ui }), width);
};
describe('<LoadingIndicator />', () => {
    ...
    it('should render blank when streamingState is Idle and no loading phrase or thought', () => {
        const { lastFrame } = renderWithContext(_jsx(LoadingIndicator, { elapsedTime: 5 }), StreamingState.Idle);
    ...
    it('should display the currentLoadingPhrase correctly', () => {
        ...
        const { lastFrame, unmount } = renderWithContext(_jsx(LoadingIndicator, { ...props }), StreamingState.Responding);
    ...
    it('should render rightContent when provided', () => {
        ...
        const { lastFrame, unmount } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps, rightContent: rightContent }), StreamingState.Responding);
        expect(lastFrame()).toContain('Extra Info');
        unmount();
    });
    it('should transition correctly between states using rerender', () => {
        const { lastFrame, rerender, unmount } = renderWithContext(_jsx(LoadingIndicator, { elapsedTime: 5 }), StreamingState.Idle);
        ...
        rerender(_jsx(StreamingContext.Provider, { value: StreamingState.Idle, children: _jsx(LoadingIndicator, { elapsedTime: 5 }) }));
    ...
    it('should not display thought icon for non-thought loading phrases', () => {
        const { lastFrame, unmount } = renderWithContext(_jsx(LoadingIndicator, { currentLoadingPhrase: "some random tip...", elapsedTime: 3 }), StreamingState.Responding);
        expect(lastFrame()).not.toContain('💬');
        unmount();
    });
    it('should truncate long primary text instead of wrapping', () => {
        const { lastFrame, unmount } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps, currentLoadingPhrase: 'This is an extremely long loading phrase that should be truncated in the UI to keep the primary line concise.' }), StreamingState.Responding, 80);
        expect(lastFrame()).toMatchSnapshot();
        unmount();
    });
    describe('responsive layout', () => {
        it('should render on a single line on a wide terminal', () => {
            const { lastFrame, unmount } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps, rightContent: _jsx(Text, { children: "Right" }) }), StreamingState.Responding, 120);
            const output = lastFrame();
        ...
        it('should render on multiple lines on a narrow terminal', () => {
            const { lastFrame, unmount } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps, rightContent: _jsx(Text, { children: "Right" }) }), StreamingState.Responding, 79);
            const output = lastFrame();
        ...
        it('should use wide layout at 80 columns', () => {
            const { lastFrame, unmount } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps }), StreamingState.Responding, 80);
            expect(lastFrame()?.includes('\n')).toBe(false);
            unmount();
        });
        it('should use narrow layout at 79 columns', () => {
            const { lastFrame, unmount } = renderWithContext(_jsx(LoadingIndicator, { ...defaultProps }), StreamingState.Responding, 79);
            expect(lastFrame()?.includes('\n')).toBe(true);
            unmount();
        });
    });
