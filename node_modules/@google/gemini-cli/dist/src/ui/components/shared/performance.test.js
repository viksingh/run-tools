...
describe('text-buffer performance', () => {
    ...
    it('should handle pasting large amounts of text efficiently', () => {
        ...
        const lines = 5000;
        const largeText = Array.from({ length: lines }, (_, i) => `Line ${i}: some sample text with many @path/to/image${i}.png and maybe some more @path/to/another/image.png references to trigger regex. This line is much longer than the previous one to test wrapping.`).join('\n');
    ...
    it('should handle character-by-character insertion in a large buffer efficiently', () => {
        const lines = 5000;
        const initialText = Array.from({ length: lines }, (_, i) => `Line ${i}: some sample text with @path/to/image.png`).join('\n');
        const viewport = { width: 80, height: 24 };
        const { result } = renderHook(() => useTextBuffer({
            initialText,
            viewport,
    ...
    it('should highlight many lines efficiently', () => {
        const lines = 5000;
        const sampleLines = Array.from({ length: lines }, (_, i) => `Line ${i}: some sample text with @path/to/image${i}.png /command and more @file.txt`);
        const start = Date.now();
        for (let i = 0; i < 100; i++) {
