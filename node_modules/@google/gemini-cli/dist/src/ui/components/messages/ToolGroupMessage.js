...
export const ToolGroupMessage = ({ toolCalls: allToolCalls, availableTerminalHeight, terminalWidth, isFocused = true, activeShellPtyId, embeddedShellFocused, borderTop: borderTopOverride, borderBottom: borderBottomOverride, }) => {
    ...
    _jsxs(Box, { flexDirection: "column", 
        /*
          ...
          cause tearing.
        */
        width: terminalWidth, paddingRight: TOOL_MESSAGE_HORIZONTAL_MARGIN, children: [visibleToolCalls.map((tool, index) => {
                ...
                return (_jsxs(Box, { flexDirection: "column", minHeight: 1, width: contentWidth, children: [isShellToolCall ? (_jsx(ShellToolMessage, { ...commonProps, activeShellPtyId: activeShellPtyId, embeddedShellFocused: embeddedShellFocused, config: config })) : (_jsx(ToolMessage, { ...commonProps })), _jsxs(Box, { borderLeft: true, borderRight: true, borderTop: false, borderBottom: false, borderColor: borderColor, borderDimColor: borderDimColor, flexDirection: "column", borderStyle: "round", paddingLeft: 1, paddingRight: 1, children: [tool.status === ToolCallStatus.Confirming &&
                                    isConfirming &&
                                    tool.confirmationDetails && (_jsx(ToolConfirmationMessage, { callId: tool.callId, confirmationDetails: tool.confirmationDetails, config: config, isFocused: isFocused, availableTerminalHeight: availableTerminalHeightPerToolMessage, terminalWidth: contentWidth - TOOL_CONFIRMATION_INTERNAL_PADDING })), tool.outputFile && (_jsx(Box, { children: _jsxs(Text, { color: theme.text.primary, children: ["Output too long and was saved to: ", tool.outputFile] }) }))] })] }, tool.callId));
            }), 
            /*
                  We have to keep the bottom border separate so it doesn't get
                  drawn over by the sticky header directly inside it.
                 */
            (visibleToolCalls.length > 0 || borderBottomOverride !== undefined) && (_jsx(Box, { height: 0, width: contentWidth, borderLeft: true, borderRight: true, borderTop: false, borderBottom: borderBottomOverride ?? true, borderColor: borderColor, borderDimColor: borderDimColor, borderStyle: "round" })), (borderBottomOverride ?? true) && visibleToolCalls.length > 0 && (_jsx(ShowMoreLines, { constrainHeight: constrainHeight }))] }));
};
