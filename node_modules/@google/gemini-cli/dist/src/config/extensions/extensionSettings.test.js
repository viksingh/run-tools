...
describe('extensionSettings', () => {
    ...
    describe('updateSetting', () => {
        ...
        it('should throw error if env var name contains invalid characters', async () => {
            ...
            await expect(updateSetting(securityConfig, '12345', 'VAR-BAD', mockRequestSetting, ExtensionSettingScope.USER, tempWorkspaceDir)).rejects.toThrow(/Invalid environment variable name/);
        });
        it('should throw error if env var value contains newlines', async () => {
            mockRequestSetting.mockResolvedValue('value\nwith\nnewlines');
            await expect(updateSetting(config, '12345', 'VAR1', mockRequestSetting, ExtensionSettingScope.USER, tempWorkspaceDir)).rejects.toThrow(/Invalid environment variable value/);
        });
