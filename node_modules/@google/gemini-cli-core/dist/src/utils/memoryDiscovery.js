...
async function getGeminiMdFilePathsInternalForEachDir(dir, userHomePath, debugMode, fileService, folderTrust, fileFilteringOptions, maxDirs) {
    ...
    for (const geminiMdFilename of geminiMdFilenames) {
        ...
        const globalMemoryPath = normalizePath(path.join(globalGeminiDir, geminiMdFilename));
        ...
        if (dir && folderTrust) {
            ...
            while (currentDir &&
                ...
                const potentialPath = normalizePath(path.join(currentDir, geminiMdFilename));
                try {
...
export async function readGeminiMdFiles(filePaths, debugMode, importFormat = 'tree') {
...
export async function getGlobalMemoryPaths(debugMode = false) {
...
export function getExtensionMemoryPaths(extensionLoader) {
...
export async function getEnvironmentMemoryPaths(trustedRoots, debugMode = false) {
...
export function categorizeAndConcatenate(paths, contentsMap, workingDir) {
    const getConcatenated = (pList) => concatenateInstructions(pList
        .map((p) => contentsMap.get(p))
        .filter((c) => !!c), workingDir);
    return {
...
export async function loadServerHierarchicalMemory(currentWorkingDirectory, includeDirectoriesToReadGemini, debugMode, fileService, extensionLoader, folderTrust, importFormat = 'tree', fileFilteringOptions, maxDirs = 200) {
    ...
    const realCwd = normalizePath(await fs.realpath(path.resolve(currentWorkingDirectory)));
    ...
    const [discoveryResult, extensionPaths] = await Promise.all([
        getGeminiMdFilePathsInternal(currentWorkingDirectory, includeDirectoriesToReadGemini, userHomePath, debugMode, fileService, folderTrust, fileFilteringOptions || DEFAULT_MEMORY_FILE_FILTERING_OPTIONS, maxDirs),
    ...
    }, contentsMap, currentWorkingDirectory);
    return {
